...
// Binary Search Tree class with binary search
public class binSearch {
    ...
    // Method to search for a key using binary search
    public Node binSearch(int key) {
        ...
        while (trav != null) {
            ...
            if (key < trav.data) {
                trav = trav.left;
            } else if (key > trav.data) { // If key is greater, go right
                trav = trav.right;
            } else { // If key matches, return node
                return trav;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        Node result = bst.binSearch(key);
        System.out.println("Binary Search for " + key + ": " +
                (result != null ? "Found " + result.data : "Not found"));

        key = 99; // Search for a value not in the tree
        result = bst.binSearch(key);
        System.out.println("Binary Search for " + key + ": " +
                (result != null ? "Found " + result.data : "Not found"));
    }
}